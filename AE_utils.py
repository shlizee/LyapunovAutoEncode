...
def interpolate_LEs(start_size, target_size, prefix='lstm', suffix = '', epoch=10, model_type='lstm', le_dir=''):
    ...
    file_name = f'{le_dir}/{model_type}_{start_size}_allLEs_e{epoch}.p'
    #
...
def combine_sizes(start_sizes, target_size, prefix='lstm', suffix = '', num_params = 1,
                  save_dir='lstm', model_type= 'lstm', load_dir='', epochs=[10], trial_dir=''):
    ...
    for start_size in start_sizes:
        for epoch in epochs:
            le_temp = interpolate_LEs(start_size, target_size, prefix, suffix, epoch, model_type, le_dir=load_dir).cpu()
            # remove nan
            ...
            val_data_temp = torch.load(f'{trial_dir}/Data/{prefix}_{start_size}{suffix}_valLoss.p').cpu()
            params_temp = torch.load(f'{trial_dir}/Data/{prefix}_{start_size}{suffix}_params.p').unsqueeze(dim=0)
            # plt.figure()
    ...
    # processed_dir = f'{le_dir}/Processed'
    torch.save(le_data, f'{save_dir}/{prefix}_{suffix}allLEs.p')
    torch.save(val_data, f'{save_dir}/{prefix}_{suffix}allValLoss.p')
    torch.save(params, f'{save_dir}/{prefix}_{suffix}allParams.p')
...
def main(args):
    ...
    trial_dir = f'{task_type}/trials/{model_type}'
    load_dir = f'{task_type}/LE_stats'
    save_dir = f'{task_type}/Processed'
    # no_evals = 300
    sizes = [64, 128, 256, 512]
    # sizes = [512]
    # print(dir)
    for size in sizes:
        extract_trials(size, trial_dir, model_type=model_type,task_type=task_type)
    combine_sizes(sizes, 1024, prefix=f'{model_type}', num_params=no_evals, save_dir=save_dir, model_type=model_type,
                  load_dir=load_dir, trial_dir=trial_dir)
    data = torch.load(f'{save_dir}/{model_type}_allLEs.p')
    targets = torch.load(f'{save_dir}/{model_type}_allValLoss.p')
    ...
    val_split = 0.1
    test_split = 0.2
    dataset_path = f'{save_dir}/{model_type}_data_split_vfrac{val_split}_testfrac{test_split}.p'
    print("")
