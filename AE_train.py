...
def ae_train(model_type, model, train_data, train_targets, val_data, val_targets, batch_size = 64, val_batch_size=64,
             alpha= 1, epochs = 100, verbose = True, print_interval = 10, suffix = '',
             device = torch.device('cpu'), dir=''):
    ...
    if not os.path.exists(f'{dir}/Latent_{model.latent_size}'):
        os.mkdir(f'{dir}/Latent_{model.latent_size}')
    torch.save(model, f'{dir}/Latent_{model.latent_size}/ae_prednet_{model.global_step}{suffix}.ckpt')
...
def main(args):
    ...
    data_dir = f'{task}/Processed'
    val_split = 0.1
    test_split = 0.2
    dataset_path = f'{data_dir}/{model_type}_data_split_vfrac{val_split}_testfrac{test_split}.p'
    ...
    split_lstm = torch.load(f'{dataset_path}')
    ...
    # model_type = "all"
    directory = f'{task}/AE_Models/{model_type}'
    if not os.path.isdir(directory):
        os.mkdir(directory)
    model = AEPredNet(latent_size = latent_size, input_size=x_train.shape[-1], lr=lr, act='tanh', device=device,
                      prediction_loss_type='MSE')
    for alpha in alphas:
        ae_train(model_type, model, x_train, y_train, x_val, y_val, alpha=alpha, epochs=epochs, print_interval=250,
                 batch_size=128, device=device, dir=directory)
