...
def calc_LEs_an(*params, model, k_LE=100000, rec_layer=0, kappa=10, diff=10, warmup=10, T_ons=1, save_s=False):
    ...
    for xt in tqdm(x_in.transpose(0, 1)[:warmup]):
        ...
        if rec_layer == 'rnn':
            ...
            J = asrnn_jac(model.rnn_layer.all_weights, ht, xt, bias=bias, eps=getattr(model.rnn_layer, 'eps'),
                          gamma=getattr(model.rnn_layer, 'gamma_I'))
        elif rec_layer == 'cornn':
            J = cornn_jac(model.rnn_layer.all_weights, ht, ct, xt, gamma=getattr(model.rnn_layer, 'gamma'),
                          dt=getattr(model.rnn_layer, 'dt'))
        else:
...
def asrnn_jac(params_array, h, x, bias, eps, gamma):
    ...
    for layer in range(num_layers):
        if layer > 0:
            x_l = h_out[layer - 1]
            x_in.append(x_l)
        y.append(((U[layer] - U[layer].transpose(1, 0) - gamma) @ h_in[layer] + W[layer] @ x_in[layer] + b[
            layer].repeat(batch_size, 1).t()).t())
        h_out.append(h_in[layer] + eps * torch.tanh(y[layer]).t())
        J_h = eps * sech(y[layer]) ** 2 @ (U[layer] - U[layer].transpose(1, 0) - gamma) + torch.eye(hidden_size).to(
            device)
        J[:, layer * hidden_size:(layer + 1) * hidden_size, layer * hidden_size:(layer + 1) * hidden_size] = J_h
        if layer > 0:
            J_xt = sech(y[layer]) ** 2 @ W[layer]
...
def cornn_jac(params_array, hy, hz, x, gamma, dt, bias=False):
    if bias:
        W, U, b_i, b_h = param_split(params_array, bias)
    else:
        W, U = param_split(params_array, bias)
    num_layers, batch_size, hidden_size = hy.shape

    V = W[0]
    wy = U[0][:hidden_size, :]
    wz = U[0][hidden_size:, :]

    hy_ = hy.transpose(1, 2)
    hz_ = hz.transpose(1, 2)
