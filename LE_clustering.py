...
def pca(latent_size, dim=2, model_type = 'lstm', no_evals = 300, v_frac= 0.2, suffix = '', thresh = 1.75):
	...
	for idx, size in enumerate(sizes):
		...
		im = ax.scatter(y[:,0][val_mask*target_mask*s_idx], y[:,1][val_mask*target_mask*s_idx], s = 20, c = 'lime', alpha = 0.5)
		im = ax.scatter(y[:,0][val_mask*~target_mask*s_idx], y[:,1][val_mask*~target_mask*s_idx], s = 20, c = 'r', alpha = 0.5)
...
def size_dist(model_type = 'lstm', dir = 'lstm/', no_evals = 300, v_frac= 0.2, suffix = '', thresh = 1.75):
	...
	i_list = torch.arange(len(sizes)*no_evals)
	splits = [(i_list>=torch.ones_like(i_list)*indices[i])*(i_list<torch.ones_like(i_list)*indices[i+1]) for i in range(len(indices)-1)]
	val_mask = torch.ones(1200, dtype = torch.bool)
	print(target_mask.float().mean())
	# val_mask.index_fill_(0, split['val_idx'].sort()[0], True)
	f = plt.figure()
	good_counts = []
	bad_counts = []
	...
	xs = [1, 2, 3, 4]
	plt.bar(xs, good_counts, color = 'lime', width = 0.6)
	plt.bar(xs, bad_counts, bottom = good_counts, color = 'r', width = 0.6)
...
if __name__ == "__main__":
	...
	thresh = 1.75
	# pca(latent_size = latent_size, dim = 2, model_type = model_type, no_evals = 300, v_frac = 0.2, suffix = 'LSTM2_', thresh = thresh)
	# param_dist(thresh = thresh)
	size_dist(thresh = thresh, suffix = suffix)
